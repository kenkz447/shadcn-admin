...
function TabsTrigger({
  ...
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        "relative inline-flex h-(--tabs-item-height) flex-1 items-center justify-center gap-(--tabs-item-gap) rounded-(--tabs-item-radius) border border-(--tabs-item-border) px-(--tabs-item-padding-x) py-(--tabs-item-padding-y) text-sm font-medium whitespace-nowrap text-(--tabs-item-fg) transition-all group-data-[orientation=vertical]/tabs:w-full group-data-[orientation=vertical]/tabs:justify-start hover:bg-(--tabs-item-bg-hover) hover:text-(--tabs-item-fg-hover) focus-visible:border-(--tabs-item-border-color-focus) focus-visible:ring-(length:--tabs-item-ring-width-focus) focus-visible:ring-(--tabs-item-ring-color-focus) focus-visible:outline-1 focus-visible:outline-(--tabs-item-border-color-focus) disabled:pointer-events-none disabled:opacity-50 group-data-[variant=default]/tabs-list:data-[state=active]:shadow-sm group-data-[variant=line]/tabs-list:data-[state=active]:shadow-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
